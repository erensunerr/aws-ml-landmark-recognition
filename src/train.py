...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False, lr_gamma=0.9):
    ...
    scheduler  = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=lr_gamma) # YOUR CODE HERE
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(
            "Epoch: {} \tTraining Loss: {:.6f} \tValidation Loss: {:.6f} \t Learning Rate: {:.6f}".format(
                epoch, train_loss, valid_loss, scheduler.get_last_lr()[-1],
            )
        )
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            print(f"Predicted class {pred.shape}, {pred}")
